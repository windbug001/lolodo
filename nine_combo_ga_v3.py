# =============================================================================
# 🐉 九組合媽媽龍策略 - 基因演算法持續優化版 v3.0
# =============================================================================
# v3.0 新增功能：
# ✅ 1. 策略權重由基因控制（不是固定 1/9）
# ✅ 2. 加入 trail_stop, take_profit, position_limit 參數
# ✅ 3. 持股比例會跳動（保留原始分數權重，多策略疊加）
# =============================================================================
# 持股比例至少 3%，否則 0%
# 使用環境變數控制執行模式
# =============================================================================
...
║              🐉 九組合媽媽龍策略 - 基因演算法 v3.0                             ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  🔥 v3.0 新增: trail_stop, take_profit, position_limit                       ║
║  🔥 v3.0 新增: 持股比例跳動（策略權重由基因控制，多策略疊加）                   ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
class GeneDecoder:
    ...
    def decode(genes: List[float]) -> Dict[str, Any]:
        ...
        # 風控參數 (174-179) - 6個參數
        p['stop_loss'] = genes[174] * 0.2 + 0.1        # 10%-30% 停損
        p['trail_stop'] = genes[175] * 0.3 + 0.15     # 15%-45% 移動停損
        p['take_profit'] = genes[176] * 0.5 + 0.3     # 30%-80% 停利
        p['position_limit'] = genes[177] * 0.25 + 0.15  # 🔥 v3.0: 15%-40% 單股上限
...
def combine_strategies(params: Dict) -> pd.DataFrame:
    """
    🔥 v3.0: 組合九個策略，確保持股比例跳動

    關鍵改進：
    1. 策略權重由基因控制（w1-w9），不是固定 1/9
    2. 保留原始分數權重，不轉為等權重
    3. 同一支股票被多策略選中時會疊加（產生持股比例跳動）
    """
    ...
    # 🔥 v3.0: 使用 reduce 合併，確保 fill_value=0 正確處理
    from functools import reduce
    combined = reduce(lambda a, b: a.add(b, fill_value=0), positions)

    # ⚠️ 關鍵：確保每股至少 3%，否則設為 0（但保留原始相對權重差異）
    n_stocks = params.get('n_stocks', 20)
    max_stocks = int(1 / MIN_POSITION_RATIO)  # 最多 33 檔 (3% 下限)
    actual_n = min(n_stocks, max_stocks)

    def normalize_preserve_ratio(row):
        """
        🔥 v3.0: 保留原始權重比例的正規化
    ...
    # 🔥 v3.0: 應用保留比例的正規化
    position = combined.apply(normalize_preserve_ratio, axis=1)
...
def run_backtest(position, params, start_date=None, end_date=None, name="Strategy", upload=False):
    ...
    try:
        ...
        # 🔥 v3.0: 加入 trail_stop, take_profit, position_limit 參數
        report = sim(
            position,
            ...
            trail_stop=params.get('trail_stop', 0.35),      # 🔥 v3.0 新增
            take_profit=params.get('take_profit', 0.70),    # 🔥 v3.0 新增
            position_limit=params.get('position_limit', 0.35),  # 🔥 v3.0 新增
