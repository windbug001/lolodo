...
# 第十四部分：回測歷史前五名並上傳
# =============================================================================
def backtest_top5_history():
    """
    回測歷史 Pareto Archive 前五名並上傳到 FinLab
    檔名使用全英文
    """
    print(f"\n{'='*60}")
    print("📊 回測歷史前五名策略並上傳 FinLab")
    print(f"{'='*60}")

    # 檢查是否有歷史存檔
    if not pareto_archive.archive:
        print("⚠️ 無歷史存檔，跳過歷史回測")
        return []
    ...
    if not sorted_archive:
        print("⚠️ 歷史存檔為空，跳過歷史回測")
        return []

    print(f"📋 找到 {len(sorted_archive)} 個歷史最佳個體")
    results = []

    for rank, elite in enumerate(sorted_archive, 1):
        try:
            print(f"\n--- 回測第 {rank} 名 ---")
            genes = elite['genes']
            ...
            if position is None or position.empty:
                print(f"   ⚠️ 第 {rank} 名持倉為空，跳過")
                continue

            # 計算預期分數
            expected_sharpe = fitness[0] / 5.0 * TARGET_SHARPE
            expected_capacity = fitness[1] / 5.0 * MIN_CAPACITY / 1e4

            # 全英文檔名
            strategy_name = f"NineComboDragon_GA_v5_Top{rank}_W{WINDOW_ID}"

            print(f"   📈 預期夏普: {expected_sharpe:.2f}")
            print(f"   💰 預期胃納量: {expected_capacity:.0f} 萬")
            print(f"   📝 上傳名稱: {strategy_name}")
            ...
            metrics = report.get_metrics()
            actual_sharpe = metrics['ratio'].get('sharpeRatio', 0) or 0
            actual_capacity = metrics['liquidity'].get('capacity', 0) or 0
            annual_return = metrics['profitability'].get('annualReturn', 0) or 0

            print(f"   ✅ 上傳成功!")
            print(f"      實際夏普: {actual_sharpe:.2f}")
            print(f"      實際胃納量: {actual_capacity/1e4:.0f} 萬")
            print(f"      年化報酬: {annual_return*100:.1f}%")
    ...
    if results:
        ...
        print("📊 歷史前五名回測彙總")
        print(f"{'='*60}")
        print(f"{'排名':<6}{'名稱':<35}{'夏普':<10}{'胃納量(萬)':<12}")
        print("-" * 60)
        for r in results:
            print(f"{r['rank']:<6}{r['name']:<35}{r['sharpe']:<10.2f}{r['capacity']/1e4:<12.0f}")
...
╚════════════════════════════════════════════════════════════════╝
    ...
    # 🔥 一開始先回測歷史前五名並上傳 FinLab
    history_results = backtest_top5_history()
    if history_results:
        print(f"\n✅ 已完成 {len(history_results)} 個歷史策略的回測與上傳")
    print(f"\n{'='*60}")
    print("🚀 開始新一輪 GA 演化優化...")
    print(f"{'='*60}")
