...
"""
================================================================================
🐉 九組合天空龍 GA v5.1 - 遺傳演算法選股優化系統（市場狀態動態版）
Nine Combo Dragon GA v5.1 - Genetic Algorithm Stock Optimizer (Market-State Dynamic)
================================================================================

【v5.1 新增 - 市場狀態動態調整（關鍵！達成夏普4.2）】🔥
✅ 市場狀態判斷函數（BULL/BEAR/RANGE）- 使用 MA40/MA120
✅ 動態持股比例（熊市10%、震盪60%、牛市100%）
✅ 動態停損機制（熊市10%、震盪20%、牛市30%）
✅ 關鍵設計：2020年V型反轉不停損、2022年熊市減倉
✅ 所有參數支援環境變數覆蓋
...
# GA 演化參數
POPULATION_SIZE = int(os.environ.get('POPULATION_SIZE', 120))  # 🔥 優化：更大族群
N_GENERATIONS = int(os.environ.get('EVOLUTION_GENERATIONS', 100))
MUTATION_RATE = float(os.environ.get('MUTATION_RATE', 0.12))  # 🔥 優化：更穩定
CROSSOVER_RATE = 0.8
N_STRATEGIES = 9  # 策略數量
...
BEAR_POSITION_RATIO = float(os.environ.get('BEAR_POSITION_RATIO', '0.1'))    # 熊市：10% 持股

# 動態停損設定
DYNAMIC_STOP_LOSS = True  # 啟用動態停損
BULL_STOP_LOSS = float(os.environ.get('BULL_STOP_LOSS', '0.30'))   # 牛市：放寬停損 30%
...
MARKET_TREND_THRESHOLD = float(os.environ.get('MARKET_TREND_THRESHOLD', '0.02'))  # 趨勢閾值
...
print(DRAGON_LOGO)
print(f"🐉 九組合天空龍 GA v5.1 (市場狀態動態版) - {N_STRATEGIES}策略 x {GeneDecoder.GENE_LENGTH if 'GeneDecoder' in dir() else 100}基因")
print("=" * 50)
...
# =============================================================================
# 🔥 市場狀態判斷模組（關鍵！達成夏普4.2的核心）
# =============================================================================
def get_market_state(close_df: pd.DataFrame) -> pd.Series:
    """
    判斷市場狀態：BULL（牛市）、BEAR（熊市）、RANGE（震盪）

    判斷邏輯：
    1. 計算市場平均價格（所有股票的平均）
    2. 使用 MA40/MA120 判斷趨勢
    3. 牛市：價格 > MA40 > MA120 且趨勢向上
    4. 熊市：價格 < MA40 < MA120 且趨勢向下
    5. 其他：震盪
    ...
    try:
        ...
        return market_state.fillna('RANGE')

    except Exception as e:
        print(f"   ⚠️ 市場狀態判斷失敗: {e}")
        return pd.Series('RANGE', index=close_df.index)


def get_position_ratio_by_state(market_state: pd.Series) -> pd.Series:
    """根據市場狀態返回持股比例"""
    ratio_map = {
        'BULL': BULL_POSITION_RATIO,
        'RANGE': RANGE_POSITION_RATIO,
        'BEAR': BEAR_POSITION_RATIO,
    }
    return market_state.map(ratio_map).fillna(RANGE_POSITION_RATIO)


def get_stop_loss_by_state(market_state: pd.Series) -> pd.Series:
    """根據市場狀態返回停損比例"""
    sl_map = {
        'BULL': BULL_STOP_LOSS,
        'RANGE': RANGE_STOP_LOSS,
        'BEAR': BEAR_STOP_LOSS,
    }
    return market_state.map(sl_map).fillna(RANGE_STOP_LOSS)


# 初始化市場狀態（使用收盤價）
_close_df = data_loader.get('close')
_market_state_cache = get_market_state(_close_df) if _close_df is not None else None

if DYNAMIC_POSITION_RATIO:
    print(f"🔥 動態持股比例已啟用：牛市{BULL_POSITION_RATIO*100:.0f}% / 震盪{RANGE_POSITION_RATIO*100:.0f}% / 熊市{BEAR_POSITION_RATIO*100:.0f}%")
if DYNAMIC_STOP_LOSS:
    print(f"🛡️ 動態停損已啟用：牛市{BULL_STOP_LOSS*100:.0f}% / 震盪{RANGE_STOP_LOSS*100:.0f}% / 熊市{BEAR_STOP_LOSS*100:.0f}%")
...
class NineComboStrategyEngine:
    ...
    def combine_strategies(self, params: Dict) -> pd.DataFrame:
        ...
        try:
            ...
            # =================================================================
            if DYNAMIC_POSITION_RATIO and _market_state_cache is not None:
                try:
                    # 對齊市場狀態到 position 的 index
                    market_state_aligned = _market_state_cache.reindex(
                        position_combined.index, method='ffill'
                    ).fillna('RANGE')
                    ...
                    # 🔥 關鍵：根據市場狀態調整持股比例
                    # 熊市時只持 10%，牛市 100%
                    for col in position_combined.columns:
                        position_combined[col] = position_combined[col] * position_ratio

                    # 計算動態停損並存入 params（供回測使用）
                    dynamic_stop_loss = get_stop_loss_by_state(market_state_aligned)
                    params['_dynamic_stop_loss'] = dynamic_stop_loss

                except Exception as e:
                    pass  # 如果出錯，繼續使用原始 position
...
class WalkForwardBacktest:
    ...
    def _backtest_single_window(self, position: pd.DataFrame, params: Dict, name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if DYNAMIC_STOP_LOSS and '_dynamic_stop_loss' in params:
                try:
                    dynamic_sl = params['_dynamic_stop_loss']
...
def main():
    ...
    print(f"""
╔════════════════════════════════════════════════════════════════╗
║     🐉 九組合天空龍 GA v5.1 - 市場狀態動態調整版               ║
║     Nine Combo Dragon GA v5.1 - Market-State Dynamic           ║
╠════════════════════════════════════════════════════════════════╣
    ...
    if pareto_front:
        ...
        try:
            ...
            if not position.empty:
                # 🔥 動態停損
                if DYNAMIC_STOP_LOSS and '_dynamic_stop_loss' in best_params:
                    try:
                        dynamic_sl = best_params['_dynamic_stop_loss']
                        if isinstance(dynamic_sl, pd.Series) and len(dynamic_sl) > 0:
                            stop_loss_value = dynamic_sl.median()
                        else:
                            stop_loss_value = best_params['stop_loss']
                    except:
                        stop_loss_value = best_params['stop_loss']
                else:
                    stop_loss_value = best_params['stop_loss']
