...
# 🔐 API Key 取得順序：1. 環境變數 2. Colab Secrets 3. 預設值
...
╚════════════════════════════════════════════════════════════════╝
    ...
    if pareto_archive.archive:
        ...
        best_archived = max(pareto_archive.archive, key=lambda x: sum(x['fitness']))
        print(f"   歷史最佳綜合適應度: {sum(best_archived['fitness']):.4f}")

        try:
            # 解碼基因
            best_params = gene_decoder.decode(best_archived['genes'])
