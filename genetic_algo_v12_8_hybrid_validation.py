...
"""
================================================================================
🧬 六組合快快龍 基因演算法優化系統 v12.8 (純夏普修正版)
   統一功能：樣本外測試 + 斷點續傳 + 純夏普適應度
================================================================================

【v12.0~v12.6 功能】
✅ 3視窗交互取優秀基因機制
✅ 安全寫入機制 + 自動備份
✅ 訓練期/測試期分離 (2017~2022 訓練, 2023~至今 測試)
✅ CheckpointManager 斷點續傳
✅ CHECKPOINT_PATH 從指定 checkpoint 開始
✅ 每股至少 3%，否則 0%
✅ 最多持有 33 檔 (100%/3%)
...
【v12.8 修正 - 歷史夏普值存儲錯誤】🔧
✅ 修正 evaluate_historical_top20() 中的 bug
✅ 更新歷史前20時使用 train_sharpe 而非 original_sharpe
...
# 🔑 自動偵測 Colab Secrets 或環境變數
def get_api_key(key_name: str, default: str = 'YOUR_API_KEY_HERE') -> str:
    """優先從 Colab Secrets 讀取，否則從環境變數讀取"""
    # 1. 先嘗試 Colab Secrets
    try:
        from google.colab import userdata
        value = userdata.get(key_name)
        if value:
            print(f"✅ {key_name} 從 Colab Secrets 載入成功")
            return value
    except:
        pass
    # 2. 再嘗試環境變數
    ...
    if value != default:
        print(f"✅ {key_name} 從環境變數載入成功")
    return value

FINLAB_API_KEY = get_api_key('FINLAB_API_KEY')
...
# 🔥 混合驗證方案：內部 Walk-Forward 設定（在訓練期內）
INTERNAL_WF_WINDOWS = [
    # (訓練開始, 訓練結束, 測試開始, 測試結束)
...
if HYBRID_VALIDATION:
    print(f"   🔬 混合驗證方案：")
    print(f"      第一階段：內部 Walk-Forward（3個窗口）")
    print(f"         ├── 訓練 2017-2019 → 測試 2020")
    print(f"         ├── 訓練 2018-2020 → 測試 2021")
    print(f"         └── 訓練 2019-2021 → 測試 2022")
    print(f"      第二階段：最終測試（{TEST_START} ~ {TEST_END}）")
else:
...
def evaluate_fitness(gene):
    """
    🔥 v12.9 混合驗證方案：兩階段適應度評估
    第一階段：內部 Walk-Forward（2017-2022 內 3 個窗口）
    第二階段：最終測試期驗證（2023-現在）
    """
    try:
        if not HYBRID_VALIDATION:
            # 原本的純夏普模式
            return _evaluate_pure_sharpe(gene)

        # ========== 第一階段：內部 Walk-Forward ==========
        internal_sharpes = []

        for tr_start, tr_end, te_start, te_end in INTERNAL_WF_WINDOWS:
            try:
                # 只回測測試期（我們關心的是測試期表現）
                position, params = combined_strategy(gene, start_date=te_start, end_date=te_end)
                if position.empty:
                    continue
                ...
                report = sim(
                    ...
                    stop_trading_next_period=False,
                    upload=False,
                    name=f"IWF_{te_start[:4]}"
                )

                if report:
                    metrics = report.get_metrics()
                    sharpe = metrics['ratio'].get('sharpeRatio', 0) or 0
                    internal_sharpes.append(sharpe)
            except:
                continue
        ...
        internal_avg = np.mean(internal_sharpes)
        internal_std = np.std(internal_sharpes)
        internal_consistency = 1.0 - (internal_std / (internal_avg + 1e-10))

        # 至少 2 個窗口夏普 > 0
        positive_windows = sum(1 for s in internal_sharpes if s > 0)
        if positive_windows < 2 or internal_consistency < 0.3:
            return (0.0,)  # 第一階段不通過
        ...
        try:
            ...
            if final_report is None:
                return (internal_avg * 0.5,)

            final_metrics = final_report.get_metrics()
            final_sharpe = final_metrics['ratio'].get('sharpeRatio', 0) or 0
        ...
        # ========== 綜合適應度計算 ==========
        # 最終測試夏普需達內部平均的 40%+
        if final_sharpe < internal_avg * 0.4:
            penalty = 0.5  # 嚴重衰退處罰
        elif final_sharpe < internal_avg * 0.6:
            penalty = 0.8  # 輕微衰退處罰
        else:
            penalty = 1.0  # 無處罰

        # 綜合適應度 = (內部平均 * 0.4 + 最終測試 * 0.6) * 穩健性 * 衰退處罰
        fitness = (internal_avg * 0.4 + final_sharpe * 0.6) * internal_consistency * penalty
...
def _evaluate_pure_sharpe(gene):
    """原本的純夏普適應度評估（備用）"""
    try:
    ...
    for i, item in enumerate(top20[:20], 1):
        ...
        try:
            ...
            if oos_result:
                ...
                # 更新歷史前20的訓練期/測試期資訊
                # 🔥 修正：使用 train_sharpe 而不是 original_sharpe（original_sharpe 可能是舊版混合適應度）
                historical_top20_mgr.update(
                    genes=genes,
                    sharpe=train_sharpe,  # 🔥 修正：用正確的訓練期夏普值
                    train_sharpe=train_sharpe,
                    test_sharpe=test_sharpe
                )
